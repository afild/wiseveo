import type { Prisma, PrismaClient } from "@/generated/prisma_new/client"
...
/**
 * Chamar DENTRO da transação que grava. Lança DateClosedError quando algum dia ou competência
 * está fechado e não há override válido. Devolve o fechamento lido (útil para logs).
 */
export async function assertWritable(
  ...
  const days = uniq(input.days.filter(isDayKey)).filter((d) => isDayClosed(d, closing.closedThrough))
  const periods = uniq((input.periods ?? []).filter((p): p is string => typeof p === "string" && PERIOD_RE.test(p)))
    .filter((p) => isPeriodClosed(p, closing.closedThrough))
  if (days.length === 0 && periods.length === 0) return closing
  if (ctx.override) return closing
  throw new DateClosedError(days, periods, closing.closedThrough, canManageClosing(ctx))
}
...
export interface UnpaidBlockers {
  ...
  sample: Array<{ id: string; date: string; description: string | null; note: string | null; amount: number; status: string }>
}
...
export async function findUnpaidBlockers(tx: Tx, ownerId: string, fromExclusive: string | null, toInclusive: string): Promise<UnpaidBlockers> {
  ...
  const [count, sample, bounds] = await Promise.all([
    ...
    tx.transaction.findMany({
      ...
      select: { id: true, date: true, description: true, note: true, amount: true, statusLookup: { select: { name: true } } },
    }),
  ...
  return {
    ...
    sample: sample.map((s) => ({ id: s.id, date: dayKeyOfStored(s.date), description: s.description, note: s.note, amount: s.amount, status: s.statusLookup.name })),
  }
}
